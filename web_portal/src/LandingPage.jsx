...
export default function LandingPage({ onLogin, onRegisterOwner, onViewTerms, onViewPrivacy }) {
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-primary-100">
      ...
      <nav className="fixed top-0 w-full bg-white/80 backdrop-blur-md z-50 border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xl md:text-2xl font-black tracking-tighter text-secondary-900">Ruta-Go</span>
          </div>
          <div className="flex items-center gap-2 md:gap-4">
            <button onClick={onLogin} className="px-3 md:px-6 py-2.5 font-bold text-slate-600 hover:text-primary-500 transition-colors text-xs md:text-sm">
              Iniciar Sesión
            </button>
            <button onClick={onRegisterOwner} className="px-4 md:px-6 py-2.5 bg-secondary-900 text-white font-bold rounded-xl shadow-xl hover:bg-black transition-all active:scale-95 text-[10px] md:text-sm uppercase tracking-wider">
              Ser Dueño
      ...
      {/* 🔥 HERO SECTION */}
      <header className="pt-32 md:pt-40 pb-16 md:pb-20 px-6 overflow-hidden relative">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="space-y-6 md:space-y-8 text-center lg:text-left">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-primary-50 rounded-full text-primary-600 font-black text-[9px] md:text-[10px] uppercase tracking-widest border border-primary-100 mx-auto lg:mx-0">
              <Star size={14} /> El futuro del transporte huilense
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-7xl font-black text-secondary-900 leading-[1.1] tracking-tight">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <a
                ...
                className="px-8 md:px-10 py-4 md:py-5 bg-primary-500 text-white font-black rounded-2xl shadow-2xl shadow-primary-500/40 hover:bg-orange-600 transition-all active:scale-95 flex items-center justify-center gap-3 group text-base md:text-lg"
              >
                Descargar App <ChevronRight className="group-hover:translate-x-1 transition-transform" />
              </a>
              <button onClick={onRegisterOwner} className="px-8 md:px-10 py-4 md:py-5 bg-slate-50 text-slate-700 font-black rounded-2xl border border-slate-200 hover:bg-white transition-all active:scale-95 text-base md:text-lg">
                Afiliar mi vehículo
              </button>
            </div>
          </div>
          ...
          <div className="relative mt-8 lg:mt-0">
            ...
            <div className="absolute -bottom-4 -left-2 md:-bottom-6 md:-left-6 bg-white p-4 md:p-6 rounded-2xl md:rounded-3xl shadow-2xl border border-slate-100 flex items-center gap-3 md:gap-4 animate-bounce-slow">
              <div className="w-10 h-10 md:w-12 h-12 bg-green-100 rounded-xl md:rounded-2xl flex items-center justify-center text-green-600"><CheckCircle2 size={20} /></div>
              <div>
                <p className="text-[10px] text-slate-400 font-bold uppercase">Asientos Libres</p>
                <p className="text-base md:text-lg font-black text-slate-800 leading-none">12 Disponibles</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 md:py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-12 md:mb-20 space-y-4">
            ...
            <p className="text-slate-500 font-medium text-sm md:text-base">Diseñamos una herramienta pensada en la productividad y comodidad.</p>
          </div>
