...
export default function Register({ onBack, initialMode = 'owner' }) {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // 3. Registrar en el nodo de usuarios
      ...
      // 4. Si es dueño, registrar en el nodo dueños (Activación Instantánea o Pendiente según lógica de negocio)
      if (mode === 'owner') {
        const ownerRef = ref(db, `dueños/${user.uid}`);
        await set(ownerRef, "pendiente"); // Los dueños pasan por revisión
      }
        ...
        <div className="max-w-md w-full bg-white rounded-[3rem] shadow-2xl p-12 text-center space-y-8 animate-in zoom-in-95 duration-500">
          ...
          <div className="space-y-4">
            <h2 className="text-3xl font-black text-slate-800 tracking-tight">
              {mode === 'owner' ? '¡Solicitud Recibida!' : '¡Bienvenido a Ruta-Go!'}
            </h2>
            ...
            {mode === 'owner' ? (
              ...
              <div className="p-4 bg-primary-50 rounded-2xl border border-primary-100 text-xs font-bold text-primary-600 uppercase tracking-wider">
  ...
  return (
    <div className="min-h-screen bg-secondary-900 flex flex-col lg:flex-row overflow-hidden">
      ...
      <div className="flex-1 bg-white p-6 lg:p-20 flex flex-col justify-center relative animate-in slide-in-from-right-4 duration-500">
        <button
          onClick={onBack}
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              <h3 className="text-3xl font-black text-slate-800 tracking-tight">
                {mode === 'owner' ? 'Crea tu cuenta de Socio' : 'Registro de Pasajero'}
              </h3>
            ...
            <div className="flex p-1 bg-slate-100 rounded-2xl">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${
                  mode === 'passenger' ? 'bg-white text-primary-500 shadow-lg shadow-slate-200' : 'text-slate-400 hover:text-slate-600'
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${
                  mode === 'owner' ? 'bg-white text-secondary-900 shadow-lg shadow-slate-200' : 'text-slate-400 hover:text-slate-600'
                }`}
              >
          ...
          <form onSubmit={handleRegister} className="space-y-6">
            ...
            <button
              ...
              className={`w-full text-white font-black py-5 rounded-2xl shadow-2xl transition-all transform active:scale-95 flex items-center justify-center gap-3 disabled:opacity-70 disabled:cursor-not-allowed text-sm uppercase tracking-widest ${
                mode === 'owner' ? 'bg-secondary-900 hover:bg-black shadow-slate-900/30' : 'bg-primary-500 hover:bg-orange-600 shadow-primary-500/30'
              }`}
            >
              {loading ? (
...
function InputField({ label, value, onChange, type = "text", placeholder, icon, required = false }) {
  return (
    <div className="space-y-1.5 group">
      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 transition-colors group-focus-within:text-primary-500">{label}</label>
