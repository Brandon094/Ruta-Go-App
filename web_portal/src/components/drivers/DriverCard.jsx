...
import { Bus, TrendingUp, Edit3 } from 'lucide-react';
import { Badge } from '../ui/Badge';
import { Button } from '../ui/Button';

/**
 * ⚛️ Molecule: DriverCard
 * Tarjeta informativa para la gestión de conductores.
 */
  ...
  return (
    <div className="card-base p-6 rounded-[2.5rem] flex flex-col gap-6 group hover:shadow-2xl transition-all duration-500 bg-white dark:bg-[#0A1F30] border border-slate-100 dark:border-none relative">
      ...
      <div className="flex items-start justify-between">
        ...
        <Badge variant={isBlocked ? 'error' : isResting ? 'warning' : 'success'}>
      ...
      <div className="p-4 bg-slate-50 dark:bg-black/20 rounded-2xl border border-slate-100 dark:border-white/5 shadow-inner">
        <p className="text-[9px] text-slate-400 dark:text-white/40 font-black uppercase tracking-[0.2em] mb-2 px-1">Turnos Asignados</p>
        <p className="text-xs text-slate-700 dark:text-white/80 font-bold truncate px-1 italic">
