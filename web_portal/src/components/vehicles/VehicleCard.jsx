...
import { Car, Hash, Calendar, Users, Edit3, Trash2, Palette, User, Briefcase, ChevronDown, ChevronUp } from 'lucide-react';
 ...
 */
export function VehicleCard({ vehicle, drivers = [], owners = [], onEdit, onDelete, isAdmin }) {
  ...
  const assignedOwner = owners.find(o => o.id === ownerId || o.uid === ownerId);
        ...
        <div className="flex gap-4 min-w-0">
          ...
          <div className="min-w-0 text-left">
            <h4 className="font-black text-slate-800 dark:text-white text-base truncate uppercase italic leading-tight">
              {vehicle.marca || vehicle.brand ? `${vehicle.marca || vehicle.brand} ${vehicle.modelo || vehicle.model}` : (vehicle.modelo || vehicle.model || 'Modelo Desconocido')}
            </h4>
            <div className="flex items-center gap-2 mt-1">
              ...
              <p className="text-[11px] font-bold text-slate-400 dark:text-white/30 uppercase tracking-widest truncate">
                {vehicle.placa || vehicle.plate || vehicle.id}
              </p>
              {isExpanded ? <ChevronUp size={14} className="text-primary-500" /> : <ChevronDown size={14} className="text-slate-300" />}
            </div>
          </div>
        </div>
        <Badge variant={vehicle.status === 'active' || vehicle.estado === 'activo' ? 'success' : 'warning'} className="shrink-0">
          {vehicle.status || vehicle.estado || 'Activo'}
        </Badge>
      </div>
        ...
        <div className="space-y-4 animate-in slide-in-from-top-4 duration-300">
          <div className="grid grid-cols-3 gap-2 border-t border-slate-50 dark:border-white/5 pt-4">
            <div className="flex flex-col items-center text-center gap-1">
              ...
              <p className="text-[10px] font-bold text-slate-700 dark:text-white leading-none">{vehicle.year || vehicle.ano || vehicle.año || 'N/A'}</p>
            ...
            <div className="flex flex-col items-center text-center gap-1">
              ...
              <p className="text-[10px] font-bold text-slate-700 dark:text-white leading-none">{vehicle.capacity || vehicle.capacidad || 13}</p>
              <p className="text-[7px] text-slate-400 dark:text-white/20 uppercase font-black tracking-tighter">Cupos</p>
            </div>
          </div>

          {/* SOCIO / DUEÑO VINCULADO */}
          <div className={`p-3 rounded-2xl border transition-all flex items-center gap-3 ${
            ...
            <div className={`p-2 rounded-xl ${assignedOwner ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/30' : 'bg-slate-200 dark:bg-white/10 text-slate-400'}`}>
              <Briefcase size={16} />
            </div>
            <div className="flex-1 min-w-0 text-left">
              <p className="text-[8px] font-black text-slate-400 dark:text-white/20 uppercase tracking-[0.2em]">Socio / Dueño de Flota</p>
              <p className={`text-xs font-bold truncate ${assignedOwner ? 'text-slate-800 dark:text-white' : 'text-slate-400 italic'}`}>
                {assignedOwner ? (assignedOwner.name || assignedOwner.nombre || assignedOwner.email) : 'Sin socio asignado'}
              </p>
            </div>
          </div>
          ...
          <div className={`p-3 rounded-2xl border transition-all flex items-center gap-3 ${
            ...
            <div className="flex-1 min-w-0 text-left">
              ...
              <p className={`text-xs font-bold truncate ${assignedDriver ? 'text-slate-800 dark:text-white' : 'text-slate-400 italic'}`}>
                {assignedDriver ? (assignedDriver.name || assignedDriver.nombre) : 'Sin operador vinculado'}
              </p>
            </div>
